...
extern "C" {
	...
	AAPIEXPORT void * AKIActionAddDetailedSpeedAction(int ang_sectionId, int laneId, int ang_segmentId, double newSpeed, int vehTypePos, double acomplianceLevel);
	...
	AAPIEXPORT void * AKIActionAddNextSubPathPTAction(int ang_sectionId, int nbSections, int * aSections, int idline, int vehTypePos, double acomplianceLevel);
	AAPIEXPORT void * AKIActionAddCloseTurningODAction(int sectionId, int anewSection2Close, int aOrigin, int aDest, int vehTypePos, double acomplianceLevel, double visibilityDistance = 200.0, bool localEffect = true, bool isDUE = false);
	...
	AAPIEXPORT double AKIVehTypeGetPercentageForImprudentLaneChanging( int idVehicleType );
	AAPIEXPORT int AKIVehTypeSetPercentageForImprudentLaneChanging( int idVehicleType, double newPercentage );
	AAPIEXPORT double AKIVehTypeGetSensitivityForImprudentLaneChanging( int idVehicleType );
	AAPIEXPORT int AKIVehTypeSetSensitivityForImprudentLaneChanging( int idVehicleType, double newSensitivity );
	...
	AAPIEXPORT double AKIVehTypeGetPercentageForUndertaking( int idVehicleType );
	AAPIEXPORT int AKIVehTypeSetPercentageForUndertaking( int idVehicleType, double newPercentage );
	...
	struct AAPIEXPORT EquippedInfVeh{
		...
		double headway;
		//! Public Transport line identifier (-1 when the equipped vehicle does not belongs to any Public Transport Line)
		int idptline;
	};
	...
	struct AAPIEXPORT A2KSectionInf{
		...
		//! visibility distance of the reserved lanes of the section (metres or feet).
	...
	/*! \brief Turn's behaviour parameters.
	...
	struct AAPIEXPORT A2KTurnBehaviourParam{
		...
		double speedLimit;
		//! yellow box speed of the turn (km/h or mph).
		double yellowBoxSpeed;
	};
	...
	*/
	...
	AAPIEXPORT double AKIEstGetGlobalStatisticsNodeLevelOfService( int aidNode );
	AAPIEXPORT double AKIEstGetPartialStatisticsNodeLevelOfService( int aidNode );
    ...
    #define AKILegionNotLoaded              -7017
	...
	/* initPosition and initSpeed in network units (metric: m and km/h and english: feet and mph */
	AAPIEXPORT int		AKIPutVehTrafficOD(int asection, int idLane, int vehTypePos, int idCentroidOr, int idCentroidDest, double initPosition, double initSpeed, int tracking);
  ...
  */
	...
	AAPIEXPORT int AKIFleetAddFleetAssignment(int idFleet, const unsigned short *nameFleetVeh, int vehTypePos, double departureTime, double deviation);
