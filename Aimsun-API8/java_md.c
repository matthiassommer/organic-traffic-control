...
void
CreateExecutionEnvironment(int *_argc, char ***_argv, char jrepath[], jint so_jrepath, char jvmpath[], jint so_jvmpath, char **_jvmtype, char **original_argv) {
	/* Find out where the JRE is that we will be using. */
	if (!GetJREPath(jrepath, so_jrepath)) {
		ReportErrorMessage("Error: could not find Java Runtime Environment.",
			JNI_TRUE);
		exit(2);
	}
...
LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn)
{
	...
	ifn->GetCreatedJavaVMs =
		(void *)GetProcAddress(handle, "JNI_GetCreatedJavaVMs");
	if (ifn->CreateJavaVM == 0 || ifn->GetDefaultJavaVMInitArgs == 0 || ifn->GetCreatedJavaVMs == 0) {
...
GetPublicJREHome(char *buf, jint bufsize)
{
	...
	int present_jdk_major_version, present_jdk_minor_version, min_jdk_major_version, min_jdk_minor_version;
	...
	sscanf(version, "%i.%i", &present_jdk_major_version, &present_jdk_minor_version);
	sscanf(JDK_MAJOR_VERSION, "%i", &min_jdk_major_version);
	sscanf(JDK_MINOR_VERSION, "%i", &min_jdk_minor_version);
	printf("%i und %i\n", present_jdk_major_version, present_jdk_minor_version);

	if (present_jdk_major_version < min_jdk_major_version || (present_jdk_major_version == min_jdk_major_version &&
		present_jdk_minor_version < min_jdk_minor_version)) {
		fprintf(stderr, "Registry key '" JRE_KEY "\\CurrentVersion'\nhas "
			"value '%s', but at least '" DOTRELEASE "' is required.\n", version);
		RegCloseKey(key);
		return JNI_FALSE;
	}
