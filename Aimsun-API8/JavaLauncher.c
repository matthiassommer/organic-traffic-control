/*
* $Id: JavaLauncher.c,v 1.1 2010/02/25 10:04:33 hpr Exp $
*
* $Author: hpr $
*/
/* Basis dieses Codes ist java.c aus dem Java SDK 1.4.2. Die Funktion main ist in
* startJava umbenannt */
/*
...
*/
int
startJava(int argc, char ** argv, JavaVM **my_vm, JNIEnv **global_env)
{
	JavaVM *vm = 0;
	JNIEnv *env = 0;
	char *jarfile = 0;
	...
	InvocationFunctions ifn;
	char *jvmtype = 0;
	char jrepath[MAXPATHLEN], jvmpath[MAXPATHLEN], classpath[MAXPATHLEN];
	...
	/* Clear static variables, initialize */
	env = NULL;
	options = NULL;
	numOptions = 0;
	maxOptions = 0;
	...
	/* Prüfen, ob schon eine VM im Kontext dieses Prozesses läuft */
	ret = ifn.GetCreatedJavaVMs(my_vm, (jsize)1, &numberOfVMs);
	if (ret == 0 && numberOfVMs > 0) {
		/* Es läuft schon eine VM, Pointer auf VM und Env zurückgeben */
		jclass controllerClass;
		jmethodID mid;

		jReturnwert = (**my_vm)->AttachCurrentThread(*my_vm, (void **)global_env, NULL);
		if (jReturnwert < 0) {
			return (int)jReturnwert;
		}

		controllerClass = (**global_env)->FindClass(*global_env, "de/dfg/oc/otc/manager/OTCManager");
		if (controllerClass == NULL) {
			return -1;
		}

		mid = (**global_env)->GetStaticMethodID(*global_env, controllerClass, "reInitialize", "()V");
		if (mid == NULL) {
			return -1;
		}

		/* Reinitialisiert Java-GUI. Java-Seite setzt Event. */
		(**global_env)->CallStaticObjectMethod(*global_env, controllerClass, mid);
	...
	strcpy(classpath, progname);
	if ((strlen(s) + strlen(progname)) < MAXPATHLEN - 1) {
		//Länge von Classpath plus angegebener Pfad nicht zu groß
		strcat(classpath, "; ");
		strcat(classpath, s);
	}
#ifndef JAVA_ARGS
	...
	*/
	...
	/* Pointer auf Umgebung in Thread-Variable schreiben */
	*my_vm = vm;
	printf("Adresse der VM (startJava): %i\n", vm);
...
int
loadJavaLibrary(int argc, char ** argv)
{
	JavaVM *vm = 0;
	JNIEnv *env = 0;
	char *jarfile = 0;
