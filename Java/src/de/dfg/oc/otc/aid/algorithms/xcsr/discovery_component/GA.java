...
public class GA {
    ...
    public GA(LinkedHashSet<RClassifier> pop, int time, int numberActions) {
    ...
    public boolean run() {
        if ((this.time - getAverageTimeSinceLastGA()) <= XCSRConstants.theta_GA) {
            return false;
        }
        ...
        RClassifier child1 = new RClassifier(parent1);
        RClassifier child2 = new RClassifier(parent2);
        ...
        child1.prediction_error = XCSRConstants.predictionErrorReduction * ((child1.prediction_error + child2.prediction_error) / 2);
        child1.fitness = XCSRConstants.fitnessReduction * ((child1.fitness + child2.fitness) / 2);
    ...
    private RClassifier selectParent(double fitnessSum) {
        ...
        double choicePoint = XCSRConstants.drand() * fitnessSum;
        double summedF = 0.0;
    ...
    private void twoPointCrossover(RClassifier cl1, RClassifier cl2) {
        ...
        if (XCSRConstants.drand() < XCSRConstants.pX) {
            ...
            int sep1 = (int) (XCSRConstants.drand() * (length));
            int sep2 = (int) (XCSRConstants.drand() * (length)) + 1; // +1 because of int cast
     ...
     * <p>
     * NOTICE: Due to the changes needed to reach the goals of XCSR no niche mutation takes place any more.
     *
     * @param cl The classifier that shall be mutated.
     */
    private void mutateCondition(RClassifier cl) {
        ...
        for (int i = 0; i < chromo.length; i++) {
            if (XCSRConstants.drand() < XCSRConstants.pM) {
                chromo[i] += XCSRConstants.getRandomMutationValue();
            }
        }
    ...
    private void mutateAction(RClassifier cl) {
        if (XCSRConstants.drand() < XCSRConstants.pM) {
            int act;
            do {
                act = (int) (XCSRConstants.drand() * numberActions);
            } while (act == cl.action);
            cl.action = act;
        }
    }
