 ...
 */
public class ContinuousCondition {
    ...
    public ContinuousCondition(int dimension) {
     ...
     */
    public ContinuousCondition(ContinuousCondition oldCond) {
    ...
    public void setChromosome(double[] crossedChromo) {
        ...
        for (int i = 1; i <= crossedChromo.length; i++) {
            if (i % 2 == 0) {
                newChromo.add(new RealValueGene(newChromo.size(), new RealValueAllele(crossedChromo[i - 2], crossedChromo[i - 1], XCSRConstants.usedGeneRepresentation)));
            }
        }
    ...
    public boolean equals(ContinuousCondition cond) {
        if (XCSRConstants.usedGeneRepresentation == RealValueAllele.AlleleRepresentations.UNORDERED_BOUND) {
