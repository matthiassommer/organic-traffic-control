 ...
 ****************************************************************************/
package de.dfg.oc.otc.layer1.controller.xcscic.interpolation.algorithms.ext_src.liang_hale.interp;
...
import de.dfg.oc.otc.layer1.controller.xcscic.interpolation.algorithms.ext_src.liang_hale.dsp.Sampling;
import de.dfg.oc.otc.layer1.controller.xcscic.interpolation.algorithms.ext_src.liang_hale.la.DMatrix;
import de.dfg.oc.otc.layer1.controller.xcscic.interpolation.algorithms.ext_src.liang_hale.la.DMatrixQrd;
import de.dfg.oc.otc.layer1.controller.xcscic.interpolation.algorithms.ext_src.liang_hale.mesh.Geometry;
import de.dfg.oc.otc.layer1.controller.xcscic.interpolation.algorithms.ext_src.liang_hale.mesh.TetMesh;
import de.dfg.oc.otc.layer1.controller.xcscic.interpolation.algorithms.ext_src.liang_hale.util.Check;
 ...
 * Sibson (1981) also described an extend of his interpolant that is
 ...
 * </li></ul>
 *
 * (rauhdomi) Added methods to allow for the addition of nodes to an already existing mesh
 *
 * @author Dave Hale, Colorado School of Mines (edited by) Dominik Rauh
 * @version 2009.06.14, 2015.02.26
 */
public class SibsonInterpolator3 {
     ...
     * rauhdomi
     *
     * Constructs and Initializes the Interpolator with one node
     * Further nodes should be added by using the addNode() method
     */
    public SibsonInterpolator3(Method method, TetMesh.Node node)
    {
        makeMesh(node);
        freedIndices = new ArrayList<>();
     ...
     * rauhdomi
     *
     * Holds reusable indices from nodes that have been removed from the mesh
     */
    private List<Integer> freedIndices;
     ...
     * rauhdomi
     *
     * Adds a new node to a already existing mesh
     *
     * (without pushing a node on the borders of the convex hull)
     *
     * @param newNode the node to add to the mesh
     */
    public void addNewNodeNoUlp(TetMesh.Node newNode) {

        if (newNode.x() < _x1min) _x1min = newNode.x();
        if (newNode.x() > _x1max) _x1max = newNode.x();
        if (newNode.y() < _x2min) _x2min = newNode.y();
        if (newNode.y() > _x2max) _x2max = newNode.y();
        if (newNode.z() < _x3min) _x3min = newNode.z();
        if (newNode.z() > _x3max) _x3max = newNode.z();

        TetMesh.Node[] temp = _nodes.clone();
        _nodes = new TetMesh.Node[_nodes.length + 1];
        System.arraycopy(temp, 0, _nodes, 0, temp.length);

        // Add a new node to the mesh, unless one already exists there.
        boolean added = _mesh.addNode(newNode);
        Check.argument(added, "each sample has unique coordinates");
        _nodes[newNode.index] = newNode;
    }
     ...
     * @param newNode the node to add to the mesh
     */
    public void addNewNode(TetMesh.Node newNode) {
        ...
        newNode.setCoordinates(x1i, x2i, x3i);

        // Take an unused index (if exists) from a removed Node
        int newNodeIndex = 0;
        if(freedIndices.size() > 0) {
            newNodeIndex = freedIndices.get(0);
            freedIndices.remove(0);
        }
        else { newNodeIndex = _nodes.length - 1; }
        ...
        _nodes[_nodes.length - 1] = newNode;
    }

    /**
     * rauhdomi
     *
     * Removes the passed node from the mesh
     * @param nodeToRemove
     */
   public void removeNode(TetMesh.Node nodeToRemove)
   {
       boolean removed = _mesh.removeNode(nodeToRemove);
       Check.argument(removed, "was not able to remove node from mesh");

       freedIndices.add(nodeToRemove.index);
       _nodes = ArrayUtils.removeElement(_nodes, nodeToRemove);

       //recalculate bounds
       _x1min = _x2min = _x3min = Integer.MAX_VALUE;
       _x1max = _x2max = _x3max = Integer.MIN_VALUE;

       for (int i = 0; i < _nodes.length; i++) {
           if (_nodes[i].x() < _x1min) _x1min = _nodes[i].x();
           if (_nodes[i].x() > _x1max) _x1max = _nodes[i].x();
           if (_nodes[i].y() < _x2min) _x2min = _nodes[i].y();
           if (_nodes[i].y() > _x2max) _x2max = _nodes[i].y();
           if (_nodes[i].z() < _x3min) _x3min = _nodes[i].z();
           if (_nodes[i].z() > _x3max) _x3max = _nodes[i].z();
       }
   }
     ...
     * rauhdomi
     *
     * Creates a mesh for one single node
     *
     * @param node the Node to create the mesh for
     */
    private void makeMesh(TetMesh.Node node) {

        _x1min = node.x(); _x1max = node.x();
        _x2min = node.y(); _x2max = node.y();
        _x3min = node.z(); _x3max = node.z();

        _nodes = new TetMesh.Node[1];
        _mesh = new TetMesh();
            ...
            node.setCoordinates(x1i, x2i, x3i);
     ...
     * rauhdomi
     *
     * Creates a mesh for a given set of sample points
     *
     * (without pushing nodes on the convex hull)
     *
     * @param f
     * @param x1
     * @param x2
    ...
    private static class HaleLiang extends VolumeAccumulator {
        ...
        private void processFace(double xp, double yp, double zp, Face face) {
            ...
            accumulate(na, vab);
            accumulate(nb, -vab); // here we
            accumulate(nb, vbc);
            accumulate(nc, -vbc); // must go
            accumulate(nc, vca);
            accumulate(na, -vca); // both ways!
