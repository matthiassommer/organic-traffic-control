...
/**
 * Main part of the LCS-CIC. Keeps a population of classifiers and provides methods
 * to select signal plans. This LSC is based on the XCST with an added Interpolation component, which uses the XCS's
 * classifier population as sites (thus C(lassifier)IC). For a given situation a classifier with a matching signal plan
 * is interpolated. This Interpolated signal plan then influences the chosen action in the predicition array.
 * <p>
...
public class XCSCIC extends AbstractTLCSelector {
    private static final Logger log = Logger.getLogger(XCSCIC.class);
     ...
     *                       population
     */
    private XCSCIC(final Attribute attribute, final String populationFile) {
     ...
     * @param attribute the attribute handled by the LCS
     */
    public XCSCIC(final OTCNode node, final Attribute attribute) {
        ...
        this.interpolationComponent.setCorrespondingXCSCIC(this);
        this.interpolationComponent.setEvaluationFilePath(XCSCICConstants.OUT_FILE_PATH
                + node.getJunction().getId()
                + XCSCICConstants.OUT_FILE_EXTENSION);
    ...
    public final TrafficLightControllerParameters determineDesiredTLC(final float[] _situation,
        ...
        // Build match set
        final de.dfg.oc.otc.layer1.controller.xcscic.ClassifierSet matchSet = this.population.buildMatchSet(situation);
        if (matchSet.isEmpty()) {
            final de.dfg.oc.otc.layer1.controller.xcscic.Classifier classifier = this.population.selectNonMatchingClassifier(situation);
            if (classifier != null) {
        ...
        final de.dfg.oc.otc.layer1.controller.xcscic.ClassifierSet actionSet = matchSet.buildActionSet(interpolationComponent);
        final de.dfg.oc.otc.layer1.controller.xcscic.Classifier classifier = actionSet.get(0);
    ...
    public final float determinePredictionForAction(final int actionHash, final float[] _situation,
        ...
        de.dfg.oc.otc.layer1.controller.xcscic.ClassifierSet matchSet = new de.dfg.oc.otc.layer1.controller.xcscic.ClassifierSet();

        try {
            matchSet = population.buildMatchSet(situation);
            if (matchSet.isEmpty()) {
                final de.dfg.oc.otc.layer1.controller.xcscic.Classifier tmpCl = population.selectNonMatchingClassifier(situation);
                if (tmpCl != null) {
                    matchSet.add(tmpCl);
                }
            }
        ...
        final de.dfg.oc.otc.layer1.controller.xcscic.ClassifierSet tempActionSet = updateSetToCycleTime(actionSet, cycleConstraint);
    ...
    public final void distributeReward(final float reward) {
        ...
        try {
            ...
            //Melde der Interpolationskomponente wie lohnend die zuletzt ausgeführte action war
            if (afterWarmUp() && (this.latestAction != null) && (this.latestSituation != null)) {
                provideFeedbackToIC(this.latestSituation, this.latestAction, mappedReward);
            }

            // Determine numerosity sum for the current action set
            int numerositySum = 0;
            for (de.dfg.oc.otc.layer1.controller.xcscic.Classifier cl : actionSet) {
                numerositySum += cl.getNumerosity();
            }

            // Update all classifier in the action set
            for (de.dfg.oc.otc.layer1.controller.xcscic.Classifier classifier : actionSet) {
    ...
    /**
     * Reports the latest situation and executed action back to the interpolation component, in order to recalculate
     * the trust level
     *
     ...
     */
    private void provideFeedbackToIC(Situation situation,
        ...
        try {
            ...
            while ((clString = br.readLine()) != null) {
                ...
                try {
                    ...
                    de.dfg.oc.otc.layer1.controller.xcscic.Classifier.Creator clCreator;
                    int numberOfWidenings;

                    if (idCreatorParts[1].equals("EvoAlg")) {
                        clCreator = de.dfg.oc.otc.layer1.controller.xcscic.Classifier.Creator.EvoAlg;
                        numberOfWidenings = 0;
                    }
                    else if(idCreatorParts[1].equals("IC")) {
                        clCreator = de.dfg.oc.otc.layer1.controller.xcscic.Classifier.Creator.IC;
                        numberOfWidenings = 0;
                    }
                    else {
                        clCreator = de.dfg.oc.otc.layer1.controller.xcscic.Classifier.Creator.LCS;
                        numberOfWidenings = new Integer(idCreatorParts[1].substring(3, 6));
                    }

                    // Create the classifier condition
                    final de.dfg.oc.otc.layer1.controller.xcscic.ClassifierCondition clCond = new de.dfg.oc.otc.layer1.controller.xcscic.ClassifierCondition(clParts[1]);
                    ...
                    final de.dfg.oc.otc.layer1.controller.xcscic.Classifier cl = new de.dfg.oc.otc.layer1.controller.xcscic.Classifier(clCond, tlcParams, prediction);
     ...
     */
    private void logSelectAction(final float[] situation, final int cycleConstraint, final de.dfg.oc.otc.layer1.controller.xcscic.ClassifierSet matchSet,
    ...
    public final void saveMappingToFile(final String filename) {
        ...
        int numerositySum = this.population.getNumerositySum();
        log.info("Number of distinct actions in population set: " + numDistinctActions);
        log.info("Sum of numerosities: " + numerositySum);
    }
    ...
    public final TrafficLightControllerParameters selectAction(float[] situation, final int cycleConstraint)
        ...
        try {
            ...
            this.interpolationComponent.interpolate(this.latestSituation, this.population);
            ...
            String msg = ExceptionUtils.getStackTrace(t);
            throw new ClassifierException(t.getCause().getMessage());
        }
    }
    ...
    private ClassifierSet updateSetToCycleTime(final ClassifierSet classifiers, final int cycleConstraint) {
        ...
        for (Classifier classifier : classifiers) {
            ...
            clonedClassifier.setCreatedBy(de.dfg.oc.otc.layer1.controller.xcscic.Classifier.Creator.LCS);
    ...
    private TrafficLightControllerParameters selectActionBasedOnSituation(float[] situation,
        ...
        if (isSituationOk(situation)) {
            ...
            if (matchSet.isEmpty()) {
                ...
                final de.dfg.oc.otc.layer1.controller.xcscic.Classifier classifier;

                //Use Interpolation based or normal (widening) covering
                // TODO - Entscheidung (Parameter) ob dennoch Widening eingesetzt werden soll, wenn keine Interpolation möglich
                if (InterpolationConstants.INTERPOLATION_BASED_COVERING) {
                    classifier = population.selectNonMatchingClassifierByInterpolation(
                ...
                if (classifier != null) {
                    ...
                    if (InterpolationConstants.ADD_COVERED_TO_POP) {
            ...
            if (InterpolationConstants.ALWAYS_EXECUTE_INTERPOLATED_ACTION && couldInterpolate()) {
                OTCValue interpolationResult = (OTCValue) this.interpolationComponent.getInterpolatedValue();

                action = interpolationResult.getInterpolatedClassifier().getAction();
            } else if (matchSet.isEmpty()) {
                // return defaults
    ...
    private boolean couldInterpolate() {
        return ((OTCValue) interpolationComponent.getInterpolatedValue()).getInterpolatedClassifier() != null;
    }
    ...
    private Situation getInterpolationSituationBySignalGroups(final float[] situationByTurnings) {
        ...
        //Using a TreeMap because ordering the map by the signalgroup id is important for interpolation
